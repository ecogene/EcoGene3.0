...
//	$add_left_end = isset($add_left_end) ? $add_left_end : "";
//	

	if(map_sites($_REQUEST['sites'], $_REQUEST['map_magnify'], $_REQUEST['left_end'], $_REQUEST['add_left_end'], $tmpfname, &$area_map))
	{
		
		echo "<MAP NAME=\"map_site\">";
		for ($t=0;$t<count($area_map);$t++){
			echo "<AREA NAME=\"area_site".$t."\" COORDS=\"".$area_map[$t]->left.",".$area_map[$t]->top.",".$area_map[$t]->right.",".$area_map[$t]->bottom."\" title=\"".$area_map[$t]->alt."\" target=\"_parent\" HREF=\"".$area_map[$t]->link."\"".">";
		}
		echo "</MAP>";
		echo "<center><IMG src=\"$tmpfname\" usemap=\"#map_site\" border=0>";
...
class enzyme_info{
     ...
     function enzyme_info($enzyme_name, $pattern, $detected_positions) {

        $this->enzyme_name = $enzyme_name;
        
        $this->pattern = $pattern;        

        $this->detected_positions = $detected_positions;
    }

}
	
function get_sites_info($sites, $left_end, $right_end, $sites_info)
{
	global $GENOME_LENGTH, $ecogene_db, $mapsearch_db;
	
	//get the the sequence in the range
	if($left_end<=$right_end)
	{
		$query = "Select substring(s.sequence,".$left_end.",".($right_end-$left_end).") as sequence from t_sequence s";
	}
	else //consider the genenome is a circular sequence
	{
	...
	$query = "Select name, sequence, seq,  is_symmetrical FROM t_enzyme WHERE  name in (".$searchegs.") ORDER by name ASC";
	$result = mysql_query($query) or die("Query failed : " . mysql_error());
	...
	$i=0; $sub_index = 0;
 	While ($i < $number) {
		...
		$enzymes[$i] = new enzyme_info($enzyme_name, $pattern, &$detected);
			
		$i++;
 	}
}
//$map_magnify, $left_end, $add_left_end, $tmpfname, $map0fmap)
// Description: 
